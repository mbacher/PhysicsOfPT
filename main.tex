...
% Old raster figure (media/image1.png) replaced by circuitikz schematic; see changes.md #36
\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{%
\begin{circuitikz}[
...
\rev{To illustrate the eddy-current mechanism described above in its simplest form---before turning to the anatomically realistic digital-human phantom of the study simulation (Section~\ref{methods-1})---we first consider a deliberately simplified geometry.}
...
\rev{Because the simulation provides the exact cardiac phases of full expansion (frame~1) and maximum contraction (frame~7) as ground truth, modulation depth is evaluated directly between these two known extremal frames rather than as a peak-to-peak search over all frames. This differs from the experimental analysis, where the true extremal phases are unknown and the peak-to-peak excursion of the averaged waveform is used instead.}
...
\revbrown{
The PT generator, a small loop antenna integrated into the anterior coil array housing, was driven at 0\,dBm (1\,mW) at a carrier frequency of 62.5\,MHz (corresponding to 1.5\,T). Receive coil arrays consisted of either a Biomatrix Body~18 or Body~12 \rev{(Siemens Healthineers, Forchheim, Germany)} (anterior) combined with a Spine~24 array (posterior), yielding 18+9 or 12+12 active receive channels respectively.}
...
\begin{figure}
\centering
\includegraphics{./media/image13.pdf}
...
\begin{table}[htbp]
\centering
\caption{Simulation array-mean modulation depths per frequency, grouped by coil array, derived from Table~\ref{tab:per-channel}. This layout mirrors Table~\ref{tab:exp-sparams} for direct comparison with experimental results.}
