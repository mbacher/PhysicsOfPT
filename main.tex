...
\rev{In practice, the PT signal is separated from the MR imaging signal in the frequency domain. The PT amplitude and phase are extracted from a narrow spectral bin (typically $\leq 5\,\mathrm{Hz}$ bandwidth) centered at $f_{PT}$ in the digitized receiver data \cite{Speier2015PT-Nav:MR-receiver}. This narrowband extraction provides substantial noise suppression ($\mathrm{SNR} \propto \sqrt{f_s / \Delta f}$, where $f_s$ is the sampling rate and $\Delta f$ the extraction bandwidth). The present work models only the electromagnetic physics of the modulation mechanism; the signal processing aspects of PT extraction are beyond its scope.}
...
These specific frequencies were chosen based on technical considerations
for integration into the Siemens MAGNETOM receive architecture.
...
to the magnetic field generated by eddy currents.
...
versa. This secondary magnetic field \(\mathbf{H}_{\mathbf{e}}\) created
...
\begin{equation}
\label{eq:ampere-maxwell-tissue}
...
The secondary field \(\mathbf{B}_{e}\) acts against the primary field
...
\begin{equation}
\label{eq:bg-generator}
B_{G} = \frac{i_{G}}{A R_{\text{mag}}}
\end{equation}

And the total magnetic field from the primary and secondary field is
given by
...
as the sum of \(B_{G}\) and \(B_{e}\).

Performing Laplace transformation with \(s\) being the complex frequency,
and multiplication with \(A\), the magnetic flux \(\phi\) is derived
as

\begin{equation}
\label{eq:phi-laplace}
\phi = \frac{i\omega}{R_{\text{mag}}\left(1 + \mu_{0}\sigma_{\text{tissue}}s + \mu_{0}\varepsilon_{\text{tissue}}s^{2}\right)}
\end{equation}

The induced voltage is \(V = \omega\frac{\partial \phi}{\partial t}\)
and the impedance is \(Z = \frac{V}{i}\),

Thus, the impedance transfer function of the coil is
...
where \(L = \frac{\omega^{2}}{R_{mag}}\) is the constant inductance of
...
Such that the capacitance \(C_{e}\) represents the displacement current
...
Using the above model, the equivalent RLC circuit takes the form of an
inductively coupled pair of inductances - essentially a transformer with
the generating coil as the ``primary'' and the receive element(s) as the
``secondary'' sides. Ignoring non-idealities such as copper resistance
and parasitic capacitances, the eddy current effect is modeled as a
resistance \(R_{e}\) and capacitance \(C_{e}\).

\begin{figure}
\centering
\includegraphics[alt={A black background with a black square Description automatically generated with medium confidence}]{./media/image1.png}
...
\begin{figure}
\centering
\includegraphics[alt={A screenshot of a computer generated image Description automatically generated}]{./media/image2.png}
...
\(\Gamma_{\sigma 1 > 0}\) create a secondary magnetic field Δ\textbf{B}
...
the received signal \cite{BacherCardiacStudy, Bacher2020PerformanceFramework}

\begin{figure}
\centering
\includegraphics[alt={A blue and yellow circle with a red dot Description automatically generated with medium confidence}]{./media/image3.png}
...
To better visualize the changes in the magnetic field due to cardiac
motion, we plot the change in the magnetic field at every timepoint as a
difference to the first timepoint \(t_{1}\) (start of systole) in a
transverse plane intersecting the phantom's heart while also subtracting
\(B_{G}\). This is shown as an animated series in Supplementary Figure
1.

\begin{figure}
\centering
\includegraphics[alt={A screenshot of a scan of a body Description automatically generated}]{./media/image5.png}
\caption{Coronal (top) and sagittal (bottom) slices through
the heart of the virtual human phantom. (b) absolute induced current
density ‖J‖ in A/m\^{}2 and (b) absolute secondary magnetic field ‖ΔH ‖
...
1mW input power) and in Figures~\ref{fig:rx-phase-62} and~\ref{fig:rx-phase-122} as phase modulation. For complex
magnitude, modulation depths are given as percent of the direct coupling
amplitude, as defined in Eq.~\eqref{eq:mod-depth-mag}. Modulation depth for phase signals is
given simply as the difference between the phase value between maximum
expansion and maximum contraction of the cardiac volume ground truth.
...
All evaluations were performed at frequencies of 62.5 MHz and 122.5MHz,
suitable for field strengths of 1.5T and 3T, respectively.

\revred{To quantify the relationship between the simulated S-parameter modulation and cardiac motion, we computed the Pearson correlation coefficient $|R|$ between the magnitude of each channel's S-parameter curve and the total heart volume ground truth extracted from the voxel model across all 12 cardiac phases. Results are summarized in Table~\ref{tab:correlation}.}

\begin{table}[ht]
\centering
\revred{
\caption{Pearson correlation coefficient $|R|$ and magnitude modulation depth $m_{D,\text{mag}}$ between simulated S-parameter curves and heart volume ground truth, given as mean $\pm$ SD across channels.}
\label{tab:correlation}
\begin{tabular}{l|cc|cc}
\hline
 & \multicolumn{2}{c|}{\textbf{62.5 MHz (1.5T)}} & \multicolumn{2}{c}{\textbf{122.5 MHz (3T)}} \\
\textbf{Array} & $|R|$ & $m_{D,\text{mag}}$ [\%] & $|R|$ & $m_{D,\text{mag}}$ [\%] \\
\hline
Anterior (S2--S13) & $0.94 \pm 0.20$ & $0.79 \pm 0.68$ & $0.94 \pm 0.18$ & $2.00 \pm 2.14$ \\
Posterior (S14--S29) & $0.99 \pm 0.03$ & $0.85 \pm 0.79$ & $0.97 \pm 0.06$ & $1.80 \pm 2.23$ \\
\hline
All channels & $0.97 \pm 0.13$ & $0.83 \pm 0.73$ & $0.96 \pm 0.13$ & $1.89 \pm 2.15$ \\
\hline
\end{tabular}
}
\end{table}
...
Modulation depths (see also Table 1 and Figures~\ref{fig:mod-depth-bars-mag} and~\ref{fig:mod-depth-bars-phase}) were found to be
...
(individual elements summarized in Table 1 and Figures~\ref{fig:mod-depth-bars-mag} and~\ref{fig:mod-depth-bars-phase}) also
...
\begin{figure}
\centering
\includegraphics[alt={A black screen with red lines Description automatically generated}]{./media/image6.png}
...
\begin{figure}
\centering
\includegraphics[alt={A black screen with red lines Description automatically generated}]{./media/image7.png}
\caption{Received signal magnitude in dBm in anterior (top)
and posterior (bottom) Rx channels over time during one cardiac phase at
...
\begin{figure}
\centering
\includegraphics[alt={A black screen with red lines Description automatically generated}]{./media/image9.png}
\caption{Received signal phase in radians in anterior (top)
and posterior (bottom) Rx channels over time during one cardiac phase at
62.5MHz. Cardiac volume ground truth is shown in red.}
...
\begin{figure}
\centering
\includegraphics{./media/image12.emf}
...
\begin{figure}
\centering
\includegraphics[alt={A screenshot of a graph Description automatically generated}]{./media/image13.png}
...
Table~\ref{tab:per-channel} lists the signed Pearson correlation coefficient $R$ and magnitude modulation depth $m_{D,\text{mag}}$ for each receive channel at both 62.5\,MHz and 122.5\,MHz. The sign of $R$ indicates whether the received signal magnitude increases (positive) or decreases (negative) with increasing cardiac volume. Channels where $|R| > 0.99$ exhibit near-perfect linear tracking of cardiac volume changes. Notably, most channels show $|R| > 0.97$, confirming that the PT magnitude signal can serve as a reliable surrogate for cardiac volume in nearly all spatial positions. Channel S2,1 at 62.5\,MHz and S12,1 at 122.5\,MHz show reduced correlation due to the sign-flip phenomenon discussed above.
}

\begin{table}[ht]
\centering
\revred{
\caption{Per-channel signed Pearson correlation coefficient $R$ and magnitude modulation depth $m_{D,\text{mag}}$ [\%] between the simulated S-parameter magnitude and heart volume ground truth.}
\label{tab:per-channel}
\small
\begin{tabular}{l|rr|rr}
\hline
 & \multicolumn{2}{c|}{\textbf{62.5 MHz (1.5T)}} & \multicolumn{2}{c}{\textbf{122.5 MHz (3T)}} \\
\textbf{Channel} & $R$ & $m_D$ [\%] & $R$ & $m_D$ [\%] \\
\hline
\multicolumn{5}{l}{\textit{Anterior array}} \\
\hline
$S_{2,1}$  & $+0.31$ & $0.02$ & $-1.00$ & $0.11$ \\
$S_{3,1}$  & $+1.00$ & $0.08$ & $-0.99$ & $0.10$ \\
$S_{4,1}$  & $+1.00$ & $0.08$ & $-0.99$ & $0.16$ \\
$S_{5,1}$  & $+1.00$ & $0.06$ & $-0.99$ & $0.34$ \\
$S_{6,1}$  & $-1.00$ & $1.38$ & $+0.97$ & $0.20$ \\
$S_{7,1}$  & $-1.00$ & $1.40$ & $+1.00$ & $2.84$ \\
$S_{8,1}$  & $-1.00$ & $1.42$ & $+1.00$ & $6.17$ \\
$S_{9,1}$  & $-1.00$ & $0.78$ & $+1.00$ & $2.46$ \\
$S_{10,1}$ & $-1.00$ & $1.70$ & $-1.00$ & $4.78$ \\
$S_{11,1}$ & $-1.00$ & $1.25$ & $-1.00$ & $3.25$ \\
$S_{12,1}$ & $+0.99$ & $0.08$ & $-0.36$ & $0.02$ \\
$S_{13,1}$ & $-1.00$ & $1.26$ & $-1.00$ & $3.53$ \\
\hline
\multicolumn{5}{l}{\textit{Posterior array}} \\
\hline
$S_{14,1}$ & $-1.00$ & $1.23$ & $-1.00$ & $7.88$ \\
$S_{15,1}$ & $-1.00$ & $2.16$ & $-1.00$ & $3.77$ \\
$S_{16,1}$ & $-1.00$ & $1.36$ & $-1.00$ & $3.26$ \\
$S_{17,1}$ & $-1.00$ & $1.25$ & $-1.00$ & $2.23$ \\
$S_{18,1}$ & $-1.00$ & $1.09$ & $+0.78$ & $1.15$ \\
$S_{19,1}$ & $-1.00$ & $2.09$ & $+1.00$ & $1.43$ \\
$S_{20,1}$ & $-1.00$ & $1.73$ & $+0.98$ & $5.14$ \\
$S_{21,1}$ & $-1.00$ & $1.49$ & $+0.87$ & $1.25$ \\
$S_{22,1}$ & $-1.00$ & $0.83$ & $+1.00$ & $1.79$ \\
$S_{23,1}$ & $+1.00$ & $0.08$ & $-1.00$ & $0.10$ \\
$S_{24,1}$ & $+0.99$ & $0.07$ & $-1.00$ & $0.14$ \\
$S_{25,1}$ & $+0.89$ & $0.05$ & $-1.00$ & $0.26$ \\
$S_{26,1}$ & $+1.00$ & $0.09$ & $-0.94$ & $0.03$ \\
$S_{27,1}$ & $+1.00$ & $0.05$ & $-0.97$ & $0.04$ \\
$S_{28,1}$ & $+1.00$ & $0.05$ & $-0.99$ & $0.09$ \\
$S_{29,1}$ & $+1.00$ & $0.05$ & $-0.99$ & $0.30$ \\
\hline
\end{tabular}
}
\end{table}
...
attenuate the received signal \cite{BacherCardiacStudy}.
...
average at 1.5T \cite{BacherCardiacStudy}.
...
\rev{\subsubsection{Generator placement}\label{generator-placement}

The position of the PT generator coil relative to the heart is a key design parameter. In this work, the generator was placed anterior to the chest wall, directly above the heart --- consistent with practical implementations where the PT source is integrated into the anterior coil array housing. Because the primary magnetic field decays as $r^{-3}$ (magnetic dipole), the induced eddy currents and consequently the secondary field scale strongly with distance. A generator placed closer to the heart produces stronger tissue-mediated coupling and thus higher modulation depths, while a more distant placement yields more uniform illumination at the expense of sensitivity. Optimization of generator position for specific clinical applications remains an interesting topic for future work.}
...
respiratory bellows. Processing of PT motion data \cite{BacherCardiacStudy, Bacher2020PerformanceFramework} is
