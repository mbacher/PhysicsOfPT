...
typical CMRI exam. We used CST Studio's Time Domain Solver with the
following settings:
...
62.5MHz. \rev{The red curve is the total (whole-organ) heart volume ground truth (right axis ``Vol/ml''; horizontal axis in cardiac frames).}}
...
\caption{Received signal magnitude in dBm in anterior (top)
and posterior (bottom) Rx channels over time during one cardiac phase at
122.5MHz. \rev{The red curve is the total (whole-organ) heart volume ground truth (right axis ``Vol/ml''; horizontal axis in cardiac frames).}}
