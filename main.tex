...
%\revdel{Received signal curves correlate qualitatively
...
\begin{table}[htbp]
\centering
\caption{\revbrown{Strongest-channel cardiac modulation depths per volunteer: the maximum and the mean of the five most strongly modulated channels (pooled across anterior and posterior elements). These summarise the signal available to source-separation post-processing, which typically retains only the most strongly modulated channels. \revdel{Magnitude values to be completed from the processing pipeline.}}}
\label{tab:exp-topN}
\begin{tabular}{lcccc}
\hline
 & \multicolumn{2}{c}{Mag.\ mod.\ depth (\%)} & \multicolumn{2}{c}{Phase mod.\ depth (mrad)} \\
...
\revdel{As the generated field is not homogeneous, falling off with distance
...
heart as possible  - the heart should ideally lie on the axis perpendicular to the loops center, though some centimeters of tolerance are acceptable \cite{Bacher2017CardiacStudy}; experimental testing indicates that a lateral offset of around 5\,cm remains permissible in real-world use cases.
When integrating the PT generator in e.g.\ an abdominal coil array, it
must be designed in such a way that the generator is optimally located
when the coil array is positioned for cardiac imaging.}
