...
\newenvironment{revgreenblock}{\begingroup\color{green!50!black}}{\endgroup}
...
\title{\rev{The Physics of Pilot Tone}}
\author{}
\date{}
...
spatially in the scan volume. \revbrown{To validate the simulation experimentally, S-parameter measurements were additionally performed in three healthy volunteers using a clinical Pilot Tone system with product receive-coil arrays.}
...
\rev{simulated} signal modulation is \revred{quantitatively} strongly correlated to total cardiac
volume \revred{($|R| > 0.94$)}. \revbrown{In-vivo S-parameter measurements in three volunteers corroborated the simulation, with cardiac modulation detected in all receive channels and modulation depths and spatial patterns broadly consistent with the predictions.}

\subsection{Conclusion}\label{conclusion}

\rev{Our simulation framework demonstrates how inductive coupling via
conductive tissues shapes the PT signal and provides mechanistic understanding of the modulation process.} \revbrown{These predictions are confirmed by in-vivo experiments.}
...
electromagnetic motion sensor, \rev{finite integration technique (FIT)}, navigator
...
\revgreen{Our} proposed Pilot Tone navigator uses a single, continuous
...
\subsection{\revgreen{Background}}\label{background}
...
\subsection{\revgreen{Basic coupling mechanism}}\label{basic-coupling}
...
\revorange{It is important to note that the near-field condition is not merely a simplification adopted in this work but a consequence of the system design and bore geometry. In our PT implementation, the carrier frequency was constrained to lie just outside the MR imaging band so that the signal could be received by the existing MR receive chain. At 1.5\,T and 3\,T this places the PT frequency at 62.5\,MHz and 122.5\,MHz respectively, well within the near-field regime extending to $r=0.76\,m$ and $r=0.39\,m$, respectively.}
...
to the magnetic field generated by eddy currents. \revpurple{A quantitative analysis showing that the Faraday-induced electric field in the body dominates over the loop-generated electric field by an order of magnitude is provided in \appref{app:derivation}.}
...
\revpurple{The induced current density in the tissue is $\mathbf{J}_{\text{tissue}} = \gamma\,\mathbf{E}_{\text{tissue}}$, where $\gamma = \sigma + j\omega\varepsilon$ is the complex admittivity combining tissue conductivity $\sigma$ and permittivity $\varepsilon$. As all biological tissues are non-magnetic ($\mu \approx \mu_0$), the permeability of vacuum is used throughout.} \rev{Note that, as shown in the appendix, $\mathbf{J}_{src}$ is zero everywhere except the loop conductor and the electric field generated by the - electrically small PT loop - is significantly smaller than the electric field created by the induced eddy currents and we let $\mathbf{E} \approx \mathbf{E}_{tissue}$}

\revpurple{Using the Lorentz reciprocity theorem (derived in detail in the Appendix), the voltage received at coil $n$ due to the tissue-mediated coupling path can be expressed as a volume integral:}
...
\revpurple{where $\mathbf{E}_1$ is the (negligible) electric field produced by the PT source (port 1) plus the electric field generated by induced eddy currents, and $\mathbf{E}_n$ is the bulk electric field that would be produced by receive coil $n$ \rev{if it were instead driven as a transmitter}, both evaluated in the same tissue environment. The received voltage is thus the tissue admittivity-weighted overlap of transmitter and receiver electric fields - essentially the spatial ``sensitivity profile'' of the measurement.}

\rev{Physically,} the conductive domain can be thought of as the source of a secondary
...
shape to that of $L_{\sigma 1}$\rev{, the eddy-current loop induced within region $\Gamma_{\sigma 1}$; throughout, $L_{\sigma i}$ denotes the eddy-current loop forming inside conductive region $\Gamma_{\sigma i}$}. Here the secondary field
...
conductive region where they originate. \rev{The eddy-current loops induced within regions $\Gamma_{\sigma 1}$ and $\Gamma_{\sigma 2}$ are denoted $L_{\sigma 1}$ and $L_{\sigma 2}$ in the text.}}
...
\rev{To illustrate the eddy-current mechanism described above in its simplest form - before turning to the anatomically realistic digital-human phantom - we first consider a deliberately simplified geometry.}

Simulated (CST Studio 2024, \rev{Time Domain Solver}, simulation volume
...
\begin{table}[htbp]
\centering
\revorange{\caption{Representative electromagnetic tissue properties (reference values at 100\,MHz, subsequently adjusted to the exact simulation frequencies via CST's Dielectric Dispersion Fit). A subset of the 30 modeled tissue types is shown.}\label{tab:tissue-properties}}
\revorange{
\begin{tabular}{l r r}
\hline
...
\rev{Because the simulation provides the exact cardiac phases of full expansion (frame~1) and maximum contraction (frame~7) as ground truth, modulation depth is evaluated directly between these two known extremal frames. In the experimental analysis the peak-to-peak excursion of the averaged waveform is used.}
...
\revbrown{
The PT generator, a small loop antenna integrated into the anterior coil array housing, was driven at 0\,dBm (1\,mW) at a carrier frequency of 62.5\,MHz (corresponding to 1.5\,T). Receive coil arrays consisted of either a 18-channel or 12-channel surface coil array \rev{(Biomatrix Body 12 and 18, Siemens Healthineers, Forchheim, Germany)} (anterior) combined with a Spine~24 array (posterior), yielding 18+9 or 12+12 active receive channels respectively.}

%\revbrown{
%The receiver was characterized by mapping digital output values to analog input %power: signals ranging from $-120$\,dBm (noise floor) to $-35$\,dBm (onset of %receiver nonlinearity) were directly fed into each coil element's low-noise %preamplifier input, establishing a calibrated 85\,dB linear dynamic range. Scattering %parameters $S_{1,n}$\,(dB) were then computed from the known transmit power (0\,dBm) %and the calibrated received signal level in each channel.}
...
Table~\ref{tab:per-channel}\revred{, with per-channel correlation coefficients to cardiac volume ground truth listed in the same table}.
...
62.5 and 122.5\,MHz. \revred{Received signal curves show strong quantitative correlation to total cardiac volume ($|R| = 0.97 \pm 0.13$ at 62.5\,MHz, $|R| = 0.96 \pm 0.13$ at 122.5\,MHz, see Table~\ref{tab:correlation}).}
...
62.5\,MHz (0.06\% anterior vs. 0.065\% posterior)\revred{. Per-channel modulation depths and signed correlation coefficients are listed in Table~\ref{tab:per-channel}.}
...
Contrary to magnitude modulation depth, \rev{absolute} phase modulation depth \rev{(in mrad)} increased
by a factor of \rev{${\sim}$}5 in anterior elements and \rev{${\sim}$}7\rev{--8} in posterior elements when
doubling the PT frequency \rev{(individual channels in Table~\ref{tab:per-channel})}.
...
62.5\,MHz. \rev{The red curve is the total (whole-organ) heart volume ground truth.}}
...
\caption{Received signal magnitude in dBm in anterior (top)
and posterior (bottom) Rx channels over time during one cardiac phase at
122.5\,MHz. \rev{The red curve is the total (whole-organ) heart volume ground truth.}}
...
\hline
V1 & \rev{0.363} & \rev{0.218} & 141.9 & 120.8 \\
V2 & \rev{0.219} & \rev{0.156} & 539.2 & 308.9 \\
V3 & \rev{0.580} & \rev{0.416} & 604.3 & 279.5 \\
\hline
Pooled & \rev{0.580} & \rev{0.433} & 604.3 & 415.3 \\
\hline
\end{tabular}
\end{table}
% \begin{figure}[htbp]
...
\begin{figure}[htbp]
\centering
\revbrown{\includesvg[width=\columnwidth]{./media/vol_segmentation.svg}
...
\begin{figure}[htbp]
\centering
\revbrown{\includesvg[width=\columnwidth]{./media/vol_anterior_mag.svg}
...
\begin{figure}[htbp]
\centering
\revbrown{\includesvg[width=\columnwidth]{./media/vol_posterior_mag.svg}
...
(secondary side) -- in this case the measurement relies on \rev{direct-path (through-body) coupling}, conceptually similar to the experiments made by Moskalenko et al. \cite{Moskalenko1958, Moskalenko1960, Moskalenko1963OnElectroplethysmography}\rev{, though at these low frequencies the coupling is near-field and inductive rather than true wave transmission}.
...
modulation but strong phase modulation of \rev{$-190$\,mrad}.
...
\subsection{\rev{Generator placement}}\label{generator-placement}
...
\subsection{\revgreen{Future work}}\label{future-work}
...
\revgreen{Finally, it is worth noting that PT can be implemented at relatively
