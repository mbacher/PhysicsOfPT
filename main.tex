...
% \end{figure}
\end{revbrownblock}
Comparing to simulation: the simulated $S_{1,n}$ range at 62.5\,MHz was approximately $-54$ to $-95$\,dB (anterior S2--S13: $-60$ to $-82$\,dB; posterior S14--S29: $-72$ to $-95$\,dB), while measured values are 20--30\,dB weaker. This difference is expected: the simulation uses idealised port coupling ($50\,\Omega$ matched, no cable losses) and a single standardised body geometry, whereas the experiment includes cable attenuation, connector losses, impedance mismatches, and inter-subject anatomical variation. The simulated magnitude modulation depths (0.02--2.2\,\% at 62.5\,MHz, Table~\ref{tab:per-channel}) are of the same order of magnitude as the measured values (0.002--0.58\,\%), with individual channels showing good qualitative agreement. The simulation thus provides a reasonable estimate of the expected modulation depths, though exact values depend on subject anatomy, coil placement, and receive chain characteristics. Importantly, the qualitative spatial pattern --- stronger modulation in channels overlapping the heart, posterior channels showing high modulation depth despite weak coupling --- is preserved in both simulation and experiment.
...
The simulation, which includes no noise sources, receiver imperfections, or cable losses, provides an idealised estimate of the expected modulation. The measured modulation depths (pooled magnitude mean 0.09\,\%, pooled phase mean 0.11\,\%) are of the same order of magnitude as the simulated values (magnitude 0.02--2.2\,\% at 62.5\,MHz), indicating good agreement given the differences in body geometry and receive chain. Crucially, the spatial patterns of modulation are preserved: channels with strong simulated cardiac sensitivity also show strong experimental modulation, and posterior elements exhibit high modulation depth despite weak absolute coupling. The simulation thus correctly predicts \emph{which} channels carry cardiac information and the \emph{relative} modulation behaviour, while exact \emph{absolute} levels depend on subject anatomy and receive chain characteristics.
