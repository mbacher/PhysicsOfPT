...
The \rev{electromagnetic} environment is modeled as a time-harmonic system at the PT frequency ($f_{PT}=62.5\,MHz$ at $1.5\,T$ and $f_{PT}=122.5\,MHz$ at 3\,T) with $\omega=2\pi f_{PT}$.
...
At an arbitrary port $n$, the fields on the cross section can be expressed in terms of the ports voltage $V_p$ and current $I_p$ via the transmission line mode \rev{\cite{Harrington2001Time-harmonicFields}}:
...
According to \cite{Rumsey1954ReactionTheory} the power observable at port $n$ is simply the product of port voltage and port current $\rev{\langle a,b \rangle}=V_n I_n$. With current normalized to $I_n=1\,A$, Eq. \eqref{RumseyVoltage} over the Volume $V_{body}$ is therefore
...
The proportion of generated field vs. induced field can be estimated as follows: Let $|\mathbf{H}_1|=1\rev{\,\mathrm{A/m}}$ at the body surface. Then the loop generated E-fields at 1.5\,T / 62.5\,MHz are approximately
...
The relationship between $S_{n,1}$ and the \rev{transimpedance} $Z_{n,1}$ is shown below \rev{\cite{Pozar2012Microwave}}:
...
At clinical frequencies (0.5\,T, 1.5\,T and 3\,T) body noise dominates, so $R_{\rev{\mathrm{tissue}}} \rev{\gg} R_{\rev{\mathrm{coil}}}$ and $Q_{\rev{\mathrm{loaded}}} \rev{\ll} Q_{\rev{\mathrm{unloaded}}}$ \cite{Edelstein1986TheImaging}.
